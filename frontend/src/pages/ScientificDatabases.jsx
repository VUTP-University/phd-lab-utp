...
export default function ScientificDatabases() {
  ...
  const fetchContent = async () => {
    ...
    try {
      const response = await fetch(`/documents/science-dbs-${i18n.language}.md`);
      if (!response.ok) throw new Error("Content not found");
      const text = await response.text();
      setContent(text);
    } catch (err) {
      console.error("Error loading scientific databases:", err);
